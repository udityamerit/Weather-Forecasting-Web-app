...
# API_KEY = "3f4f458fc6d5cb3440d24074d29f7e82"  # Replace with your actual API key
...
# # Sidebar inputs
# st.sidebar.header(translate_text("Search Location", lang_code))
# option = st.sidebar.radio(
#     translate_text("Select search method:", lang_code),
#     (translate_text("City Name", lang_code), translate_text("Coordinates", lang_code))
# )

# if option == translate_text("City Name", lang_code):
#     city = st.sidebar.text_input(translate_text("Enter City Name", lang_code), "London")
    
#     if st.sidebar.button(translate_text("Get Weather", lang_code)):
#         # Get current weather
...
#                 st.header(translate_text(f"Current Weather in {data['name']}, {data.get('sys', {}).get('country', '')}", lang_code))
...
#                         <h3 class="centered">{translate_text("Temperature", lang_code)}</h3>
#                         <h2 class="centered">{data['main']['temp']}°C</h2>
#                         <p class="centered">{translate_text("Feels like", lang_code)}: {data['main']['feels_like']}°C</p>
#                     </div>
#                     """, unsafe_allow_html=True)
...
#                     st.markdown(f"""
#                     <div class="weather-card">
#                         <h3 class="centered">{translate_text("Condition", lang_code)}</h3>
#                         <div class="centered">
#                             <img src="{icon_url}" width="50">
#                             <h2>{translate_text(weather_description, lang_code)}</h2>
#                         </div>
#                     </div>
#                     """, unsafe_allow_html=True)
...
#                     st.markdown(f"""
#                     <div class="weather-card">
#                         <h3 class="centered">{translate_text("Details", lang_code)}</h3>
#                         <p>{translate_text("Humidity", lang_code)}: {data['main']['humidity']}%</p>
#                         <p>{translate_text("Wind", lang_code)}: {data['wind']['speed']} m/s</p>
#                         <p>{translate_text("Pressure", lang_code)}: {data['main']['pressure']} hPa</p>
#                     </div>
#                     """, unsafe_allow_html=True)
...
#                     "Feels Like": [item["main"]["feels_like"] for item in forecast_list],
#                     "Description": [item["weather"][0]["description"].capitalize() for item in forecast_list],
#                     "Icon": [item["weather"][0]["icon"] for item in forecast_list],
#                     "Humidity": [item["main"]["humidity"] for item in forecast_list],
#                     "Wind Speed": [item["wind"]["speed"] for item in forecast_list]
#                 })
                
#                 # Display 5-day forecast
#                 st.header(translate_text("5-Day Forecast", lang_code))
...
#                     title=translate_text("Temperature Forecast", lang_code),
#                     labels={"value": "Temperature (°C)", "variable": "Measurement"},
#                     color_discrete_map={"Temperature": "#FF5733", "Feels Like": "#33B5FF"}
#                 )
                
#                 fig.update_layout(
#                     xaxis_title=translate_text("Date & Time", lang_code),
#                     yaxis_title=translate_text("Temperature (°C)", lang_code),
#                     legend_title=translate_text("Metric", lang_code),
#                     plot_bgcolor="cyan",
#                     hovermode="x unified"
#                 )
...
#                 st.subheader(translate_text("Daily Summary", lang_code))
                
#                 # Get unique days
#                 forecast_df["Day"] = forecast_df["Time"].dt.date
#                 unique_days = forecast_df["Day"].unique()
                
#                 daily_cols = st.columns(len(unique_days))
                
#                 for i, day in enumerate(unique_days):
#                     # Filter data for the specific day
#                     day_data = forecast_df[forecast_df["Day"] == day]
                    
#                     # Get representative data (e.g., noon or average)
#                     if len(day_data) > 0:
#                         # Try to get data closest to noon
#                         day_data["Time_Hour"] = day_data["Time"].dt.hour
#                         day_data["Distance_From_Noon"] = abs(day_data["Time_Hour"] - 12)
#                         representative_data = day_data.loc[day_data["Distance_From_Noon"].idxmin()]
...
#                         with daily_cols[i]:
#                             icon_url = f"http://openweathermap.org/img/wn/{representative_data['Icon']}@2x.png"
                            
#                             st.markdown(f"""
#                             <div class="weather-card">
#                                 <h4 class="centered">{translate_text(date_str, lang_code)}</h4>
#                                 <div class="centered">
#                                     <img src="{icon_url}" width="50">
#                                     <p>{translate_text(representative_data["Description"], lang_code)}</p>
#                                 </div>
#                                 <p class="centered">{translate_text("High", lang_code)}: {max_temp:.1f}°C</p>
#                                 <p class="centered">{translate_text("Low", lang_code)}: {min_temp:.1f}°C</p>
#                             </div>
#                             """, unsafe_allow_html=True)
#             else:
#                 st.error(translate_text(f"Error: {data['message']}", lang_code))
                
#         except Exception as e:
#             st.error(translate_text(f"An error occurred: {e}", lang_code))

# else:  # Coordinates option
#     col1, col2 = st.sidebar.columns(2)
#     with col1:
#         lat = st.number_input(translate_text("Latitude", lang_code), value=51.5074, format="%.4f")
#     with col2:
#         lon = st.number_input(translate_text("Longitude", lang_code), value=-0.1278, format="%.4f")
